...
type
  ...
  protected
    ...
    function GetValue(vName: String; Count: Integer): String;
  ...
  published
    ...
    // разделитель целой и дес. частей
...
const
  DefValues: array[0..13] of PChar = ( // unsafe type
    ...
    'CurrencyMale=1',
    'Cop=копейка;копейки;копеек',
    'Comma=,',
    'Month1=Января;Февраля;Марта;Апреля;Мая;Июня;Июля;Августа;Сентября;' +
    'Октября;Ноября;Декабря',
    'Month2=Январь;Февраль;Март;Апрель;Май;Июнь;Июль;Август;Сентябрь;Октябрь;' +
    'Ноябрь;Декабрь',
...
function TInWord.GetValue(vName: String; Count: Integer): String;
var
  S: String;
  i, p1, p2: Integer;
begin
  S := FInWordList.Values[vName];
...
begin
  case ALevel of
    lvlThousand: l_male := False;
    lvlMillion, lvlBillion: l_male := True;
    else l_male := FMale;
  end;
  ...
  if ANum >= 10 then Result := Result + GetValue('Ten', ANum - 10 + 1)
    ...
    if (ANum in [1, 2]) and not l_male then Result := Result + GetValue('One', ANum + 10 - 1)
